...
%======================= PARAMETERS =======================%
comp = 'ZZ'; %'ZZ'; %'RR'; %'TT';
coperiod = [3 100]; % Periods to filter between
amp = 8e0;
windir = 'window3hr';
...
%         end
        ...
        [~,sta1sta2_az(nstapair)] = distance(data.stapairsinfo.lats(1),data.stapairsinfo.lons(1),data.stapairsinfo.lats(2),data.stapairsinfo.lons(2));
        ...
        sta1sta2_az_all(npairall) = sta1sta2_az(nstapair);
...
for istapair = 1: npairall
    if min(abs(sta1sta2_dist_all(istapair)-dists)) > trace_space && snr_compare(istapair) > snr_thresh ...
        ...
        plot(time(indtime(1):indtime(end)),-1*ccf_waveform_all*amp+sta1sta2_dist_all(istapair),'-k','linewidth',1); hold on;
%         plot(time(indtime(1):indtime(end)),-1*ccf_waveform_all*amp+sta1sta2_az_all(istapair),'-k','linewidth',1); hold on;
    end
end
% xlim([-500 500])
xlim(xlims)
